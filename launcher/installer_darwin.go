...
func installAssociations(executable string) error {
	...
	application := filepath.Join(home, "Applications", "epub.ts.app")
	...
	if err := os.WriteFile(filepath.Join(macosDir, "epub.ts-launcher"), []byte(launcher), 0o755); err != nil {
		return err
	}
...
<plist version="1.0">
<dict>
  <key>CFBundleIdentifier</key><string>io.github.jay-waves.epub.ts</string>
  <key>CFBundleName</key><string>epub.ts</string>
  <key>CFBundleDisplayName</key><string>epub.ts</string>
  <key>CFBundleExecutable</key><string>epub.ts-launcher</string>
