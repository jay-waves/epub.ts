...
func usageError() error {
	return errors.New("usage: epub.ts <start|daemon|status|stop|open [document]|purge>")
}
...
func startDaemon() error {
	...
	if err != nil {
		return fmt.Errorf("locate epub.ts executable: %w", err)
	}
	if err := StartBackgroundProcess(executable, "daemon"); err != nil {
		return fmt.Errorf("start epub.ts daemon: %w", err)
	}
	...
	if lastError != nil {
		return fmt.Errorf("epub.ts daemon did not become ready: %w", lastError)
	}
	return errors.New("epub.ts daemon did not become ready within 10 seconds")
}
